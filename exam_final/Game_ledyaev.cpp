...
void Player::roll_cubes()
{
	...
	for (int i = 0; i < 3; i++) {
		resource.cube[i].roll();
		std::cout << "    Cube " << i << ": " << resource.cube[i].value << std::endl;
	}

	if (resource.has_cube_white) {
		resource.cube_white.roll();
		std::cout << "    White cube: " << resource.cube_white.value << std::endl;
	}
}
...
int Player::total_cubes()
{
	...
	for (int i = 0; i < 3; i++) {
		res += resource.cube[i].value;
	}

	if (resource.has_cube_white) {
		res += resource.cube_white.value;
	}
...
void Game_ledyaev::phase1()
{
	...
	if (min_players.size() == 1) {
		...
		players[index].resource.has_cube_white = true;
	}
	else {
...
void Game_ledyaev::phase246_bones() //бросок кубиков, фаза 2, 4, 6
{
	for (int i = 0; i < players.size(); i++) {
...
void Game_ledyaev::phase246_advisor() //наставники на фазах 2, 4, 6
{
}

void Game_ledyaev::select_good(Player &player, int num) //выбор ресурса
{
	for (int i = 0; i < num; i++) {
		int select;
...
void Game_ledyaev::advisor_help(const char *advisor_name, Player &player) //Советники
{
